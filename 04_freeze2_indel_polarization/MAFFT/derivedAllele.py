...
with open(args.vcf, 'r') as vcf:
	for line in vcf:
		...
		## Save intermediate sequences file
		f_baseName = col[0]+"_"+col[1]

		make_sure_path_exists('intermediateFiles/')       # create dir

		fileName     = "intermediateFiles/"+f_baseName+"_REF_BLAST.xml"
		f_REF_BLAST  = open(fileName, 'w')
		f_REF_BLAST.write(str(REF_BLAST_output))
		f_REF_BLAST.close()

		fileName     = "intermediateFiles/"+f_baseName+"_ALT_BLAST.xml"
		f_ALT_BLAST  = open(fileName, 'w')
		f_ALT_BLAST.write(str(ALT_BLAST_output))
		f_ALT_BLAST.close()
		...
		fileName     = "intermediateFiles/"+f_baseName+".mfa"   #File to be saved!
		#fileName     = "TEMP_"+splitext(basename(args.vcf))[0]+".mfa"    #TEMP file, to be deleted!
		...
		commandMAFFT = mafft_route +" --globalpair --maxiterate 20 --inputorder --quiet "+fileName
		...
		## output alignment
		fileName     = "intermediateFiles/"+f_baseName+".mafft"
		f_ALN  = open(fileName, 'w')
		f_ALN.write(str(MAFFT_output))
		f_ALN.close()
