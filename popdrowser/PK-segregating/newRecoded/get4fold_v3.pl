			...
			for my $j (0..$#parents){
				...
				if ($parents[$j] eq $id){
					...
					foreach my $char (split //, $get_fold) {
						if ($char eq "N"){
							my $us .= ($recoded,$introns_all[$i]{size} eq "long") ? "I" : "i" ;
						} else {
							$us .= $char;
						}
					}
...
sub codonCompare {
	...
	if ($fromfold eq "NNN" || $fromfold eq "III" || $fromfold eq "iii" || $fromfold eq "UUU"){
		...
		my @split_current_conv  = split //, $current;
		...
		for (my $i=0; $i<=2; $i++){		
			if ($split_current[$i] ne $split_fromfold[$i] 
			    ...
			    && $split_fromfold[$i] ne "i") {
				if ( int($split_current_conv[$i]) > int($split_fromfold[$i]) ) {
