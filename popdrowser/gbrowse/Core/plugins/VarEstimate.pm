...
# called by gbrowse to return description of plugin
 sub description { 
	p('ON-THE-FLY VARIATION ESTIMATES ANNOTATOR');
	p('This plugin allows to recompute nucleotide variation and displays a new xyplot track with the results.');
 }
...
#		$ldsinglets       = 1;
   ...
   #################### VariScan Options
	...
	###### create one-liner
		...
		my $command="/home/mike/precomputed/variscan ".$options{"f"}.' '.$variscanopt;
#		print STDERR "Running $command\n";
#		warn $command;

		(!&runAndSave(\@cols, $command, \%results)) && do{die "Error computing variscan\n";};

		my $feature_list = &printXYplot(\%results, \%options, $time_start, $alnStart, $is_out);
...
####PRINT
	...
	sub printXYplot($$){
		...
		my $jsize = $$options{'JumpSW'};
		 
		my $time_end = time - $time_start;

		foreach my $stat (keys %$results){
			
			my $description = $stat." - Window size:".$swsize." Jump size:".$jsize." Outgroup".$is_out;
			
			$feature_list->add_type('wave' => {glyph     => 'xyplot',
									 graph_type => 'boxes',
									 key       => "On-the-fly Variation reestimate",
									 description => $description,
									 bgcolor   => 'blue',
									 height    => 50,
							  });
			my $plot = Bio::Graphics::Feature->new();
			
			foreach my $point (@{$$results{$stat}}){
			
			    my $window_val = $point->{'val'};
			    $rounded = sprintf("%0.5f", $window_val);
			    $rounded = "nan" if $stat eq 'k' && $rounded == 5;
			   	if ($rounded eq "nan" || $rounded eq "NAN"){
			    	$rounded = sprintf("%0.2f", 0);
			    }		
			    
				$plot->add_SeqFeature(Bio::Graphics::Feature->new(-seq_id=>$$options{'chr'},
																  -start =>$point->{'startrefpos'} + $alnStart ,
																  -end   =>$point->{'endrefpos'} + $alnStart ,
																  -score =>$rounded ));
			}
			
			$feature_list->add_feature($plot,'wave');
		}
		warn $feature_list;
		return $feature_list;
	}
