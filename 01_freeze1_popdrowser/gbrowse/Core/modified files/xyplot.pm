...
sub draw {
  ...
  $self->panel->startGroup($gd);
  $self->_draw_scale($gd,$scale,$min_score,$max_score,$dx,$dy,$y_origin,$self);
  $self->panel->endGroup($gd);
...
sub _draw_scale {
  my $self = shift;
  my ($gd,$scale,$min,$max,$dx,$dy,$y_origin,$foo) = @_;
...
#$_->[1] = $self->feature;
#my $bar;
#foreach my $key (keys %$self->feature) { $bar .= "|".$key;}
#$_->[1] = $bar;

$_->[1] = $_->[1]/100 if  $self->feature =~ /k_/;
$_->[1] = $_->[1]/1000 if  $self->feature =~ /pi_/;
$_->[1] = $_->[1]/1000 if  $self->feature =~ /theta_/;

$_->[1] = $_->[1]/100 if  $self->feature =~ /Dprimeabs_/;
$_->[1] = $_->[1]/1000 if  $self->feature =~ /r2_/;
$_->[1] = $_->[1]/100000 if  $self->feature =~ /D_/;
$_->[1] = $_->[1]/100000 if  $self->feature =~ /Dabs_/;
